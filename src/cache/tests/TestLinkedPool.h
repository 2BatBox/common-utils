#ifndef CACHE_TESTS_TESTLINKEDPOOL_H
#define CACHE_TESTS_TESTLINKEDPOOL_H
...
class TestLinkedPool {
	...
	using Node_t = cache::LinkedPoolNode<Value_t>;
	using LinkedPool_t = cache::LinkedPool<Node_t>;
...
public:
	...
	TestLinkedPool(TestLinkedPool&&) = delete;

	TestLinkedPool operator=(const TestLinkedPool&) = delete;
	TestLinkedPool operator=(TestLinkedPool&&) = delete;

	~TestLinkedPool() { }

	void test() noexcept {
		float mem_used = m_pool.capacity() * sizeof (Node_t);
		printf("<TestLinkedPool>...\n");
...
#endif /* CACHE_TESTS_TESTLINKEDPOOL_H */
