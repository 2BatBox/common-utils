...
#include "parsers/IpStackParser.h"
#include "parsers/SctpParser.h"
#include "../../../common-utils/src/utils/HexDumper.h"
...
public:
	...
	static void dump(FILE* out, IpStackParser packet, bool hex_dump) {
	...
	static void dump_proto(FILE* out, const IpStackParser& packet, bool hex_dump) {

		switch (packet.protocol()) {

		case Protocol::L2_ETHERNET:
			hex_dump &= DUMP_PAYLOAD_ETHERNET;
		...
		if (hex_dump) {
			binio::ByteArrayConstPtr header = packet.header();
			binio::ByteArrayConstPtr payload = packet.payload();
	...
	static void dump_ethernet(FILE* out, const IpStackParser& pkt) noexcept {
	...
	static void dump_vlan(FILE* out, const IpStackParser& pkt) noexcept {
	...
	static void dump_ipv4(FILE* out, const IpStackParser& pkt) noexcept {
	...
	static void dump_udp(FILE* out, const IpStackParser& pkt) noexcept {
	...
	static void dump_sctp(FILE* out, const IpStackParser& pkt, bool payload) noexcept {
		const Sctp::Header* hdr;
		pkt.assign(hdr);
		...
		SctpParser sctp(pkt.available_range());
		const Sctp::ChunkHeader* chunk = sctp.chunk();
		while (chunk) {
			...
			if (payload) {
				binio::ByteArrayConstPtr header = sctp.chunk_header();
				if (header) {
					fprintf(out, "    |-Header [%zu] : ", header.length());
					HexDumper::hex(header, header.length());
				}

				binio::ByteArrayConstPtr payload = sctp.chunk_payload();
				if (payload) {
					fprintf(out, "    |-Payload [%zu] : ", payload.length());
...
private:
	...
	static inline void print_header(FILE* out, const char* name, const IpStackParser& pkt) {
		binio::ByteArrayConstPtr header = pkt.header();
		binio::ByteArrayConstPtr payload = pkt.payload();
